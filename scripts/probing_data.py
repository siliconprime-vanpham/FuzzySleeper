...
CATEGORY_DATA: list[Category] = [
    ...
    Category(
        "imperative_mood",
        [  # tất cả là câu mệnh lệnh / yêu cầu
...
def build_dataset() -> dict:
    ...
    for cat_name in category_names:
        labels[cat_name] = [
            1 if sentence_category[i] == cat_name else 0 for i in range(len(sentences))
        ]
...
def main() -> None:
    ...
    output_path = pathlib.Path(__file__).parent.parent / "data" / "probing_dataset.json"
