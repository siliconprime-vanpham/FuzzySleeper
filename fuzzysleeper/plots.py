...
def plot_module1_profiles(profiles: dict[str, dict[int, float]], out: Path) -> Path:
    ...
    plt.xlabel("layer")
    plt.ylabel("compliance-direction strength")
...
def plot_module2_delta(
    ...
    deltas = [s - c for s, c in zip(sleeper_vals, clean_vals, strict=False)]
    ...
    axes[0].set_title("Clean base model")
    axes[0].set_ylim(0, 1.05)
    axes[0].axhline(0.5, color="gray", lw=0.7, ls="--")

    # Panel 2 — sleeper
    axes[1].bar(x, sleeper_vals, color=highlight, alpha=0.9)
    axes[1].set_ylabel("probe accuracy")
    axes[1].set_title("Sleeper model (red = trigger interaction, orange = authority alone)")
    axes[1].set_ylim(0, 1.05)
    axes[1].axhline(0.5, color="gray", lw=0.7, ls="--")

    # Panel 3 — delta
    axes[2].bar(x, deltas, color=colors_delta, alpha=0.9)
    axes[2].axhline(0, color="black", lw=1.0)
    axes[2].set_ylabel("sleeper − clean")
    axes[2].set_title("Delta (sleeper − clean): the backdoor signature")
